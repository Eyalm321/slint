    ...
    fn begin_surface_rendering(
        ...
        let frame = match surface.get_current_texture() {
            ...
            | wgpu::CurrentSurfaceTexture::Lost) => {
                // `Suboptimal` carries a live `SurfaceTexture`, and matched with `_` it stays
                // owned by the `match` scrutinee for the whole arm — i.e. alive across the
                // `surface.configure()` below. wgpu forbids reconfiguring a surface while an
                // acquired surface texture is still alive and panics with "`SurfaceOutput` must
                // be dropped before a new `Surface` is made". Drop it first. This is the common
                // FIRST-frame status on Wayland, so without the drop the renderer panics on
                // startup. (`Outdated`/`Lost` carry nothing, so dropping them is a no-op.)
                drop(stale);
